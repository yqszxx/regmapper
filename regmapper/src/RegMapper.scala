...
object RegField {
  implicit def rwP: ReadWriter[RegField] = macroRW
}

case class RegField(width: Int)

object RegMapperParameter {
  implicit def rwP: ReadWriter[RegMapperParameter] = macroRW
}
...
case class RegMapperParameter(
  ...
  supportMask:     Boolean,
  regMap:          Seq[(Int, Seq[RegField])]) extends SerializableModuleParameter {
...
class RegIO(val dataWidth: Int) extends Bundle {
...
class RegMapper(val parameter: RegMapperParameter)
  ...
  val regIO = parameter.regMap.flatMap(_._2).map(rf => Wire(new RegIO(rf.width)))

  val sels = parameter.regMap.map(r => io.index === r._1.U)
  val wens = VecInit(sels.map(_ && io.write))
  val rens = VecInit(sels.map(_ && io.read))
  ...
  val startEndReg = parameter.regMap.map(r => r._2.foldLeft((0, Seq[(Int, Int)]())) { case ((start, lastResult), rf) =>
    val end = start + rf.width - 1
    (end + 1, lastResult :+ (start, end))
  }._2)

  regIO.zip(startEndReg.flatten).foreach { case (rio, (start, end)) =>
    rio.wdata := io.wdata(end, start)
  }

  regIO.zip(parameter.regMap.zipWithIndex.flatMap(r => r._1._2.map(_ => r._2))).foreach { case (rio, i) =>
    rio.read := rens(i)
    rio.write := wens(i)
  }
  ...
  val rdatas = regIO.zip(idRegField).groupBy(_._2).toSeq.sortBy(_._1).map(_._2).map(r => Cat(r.map(_._1.rdata)))

  io.rdata := rdatas.zip(osels).map(r => Mux(r._2, r._1, 0.U)).reduce(_ | _)
}
